...
def export_xls(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="laporan buku.xls"' #jika ada spasi tambahkan tanda ""
    return response
...
def signup(request):
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.error(request, "Something  went't wrong, get some help")
            return redirect('signup')
    else:
...
def ubah_buku(request, id_buku):
    ...
    if request.POST:
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Data Di ubah")
...
def buku(request):
    ...
    # limit = di ahir tambah [:angka]
    #filter(kelompok_id__nama='produktif')[:3]
